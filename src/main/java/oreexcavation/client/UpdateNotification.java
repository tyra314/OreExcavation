...
public class UpdateNotification
{
	...
	public void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent event)
	{
		...
		{
			event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "THIS COPY OF " + OreExcavation.NAME.toUpperCase() + " IS NOT FOR PUBLIC USE!"));
			return;
		}
		
		try
		{
			String[] data = getNotification("http://bit.ly/2ch0eQu", true);
			...
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "An error has occured while checking " + OreExcavation.NAME + " version!"));
			...
			if(hasUpdate)
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "Update for " + OreExcavation.NAME + " available!"));
				event.player.addChatMessage(new ChatComponentText("Download: http://minecraft.curseforge.com/projects/ore-excavation"));
